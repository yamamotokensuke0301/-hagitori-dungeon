(function () {
  ...
  curatedArena.sort((left, right) => left.arenaRank - right.arenaRank).forEach((monster, index) => {
    ...
    const progress = (rank - 1) / Math.max(1, curatedArena.length - 1);
    const escalation = progress ** 2;
    const finale = progress ** 4;
    const durabilityScale = 1.5 + rank * 0.012;
    const techniqueScale = 1.3 + rank * 0.008;
    const healthScale = 1.05 + progress * 0.25 + escalation * 0.65 + finale * 1.05;
    const attackScale = 1.04 + progress * 0.15 + escalation * 0.30 + finale * 0.41;
    const defenseScale = 1.03 + escalation * 0.40 + finale * 0.27;
    const dangerScale = 1.05 + progress * 0.15 + escalation * 0.35 + finale * 0.45;
    monster.hp = Math.round(monster.hp * durabilityScale * healthScale);
    monster.attack = Math.round((monster.attack + 3 + Math.floor(rank * 0.25)) * attackScale);
    monster.defense = Math.round((monster.defense + 1 + Math.floor(rank * 0.08)) * defenseScale);
    monster.acceleration += 6 + Math.floor(rank / 8) + Math.floor(3 + progress * 10 + escalation * 16 + finale * 9);
    monster.dangerous.power = Math.round(monster.dangerous.power * techniqueScale * dangerScale);
