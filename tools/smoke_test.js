...
const innAdviceBlock = mainSource.slice(mainSource.indexOf("const INN_ADVICES"), mainSource.indexOf("let recentInnAdviceIndexes"));
...
normalDonationSave.meta.guildDonatedEquipmentIds = [];
...
elements.get("#confirmOk").listeners.click();
assert(persistedNormalDonationSave.meta.guildDonatedEquipmentIds.includes("iron_sword"), "normal equipment donation ID was not recorded in meta progress");
...
eval(read("js/main.js"));
for (let action = 0; action < 3; action += 1) elements.get("[data-arena-guard]").listeners.click();
...
arenaCarrySave.arena.actionProgress = 2;
...
eval(read("js/main.js"));
elements.get("[data-arena-enemy]").listeners.click();
assert(persistedArenaCarrySave.arena.awaitingNext && persistedArenaCarrySave.arena.actionProgress === 3, "arena finishing blow lost its partial world-turn progress");
elements.get("#arenaNextButton").listeners.click();
Math.random = arenaTurnOriginalRandom;
assert(persistedArenaCarrySave.arena.round === 2 && persistedArenaCarrySave.arena.actionProgress === 3, "arena round transition reset carried action progress");
...
assert(persistedArenaReplayRewardSave.adventurer.gold === 7 && persistedArenaReplayRewardSave.adventurer.guildPoints === 0, "arena replay did not halve gold and remove GP");
assert(persistedArenaReplayRewardSave.adventurer.experience === 9, "arena replay did not halve experience");
...
assert(persistedGoldTheftSave.adventurer.gold < 100 && escapedGoldThief.stolenGold === 100 - persistedGoldTheftSave.adventurer.gold, "gold thief did not steal and retain the player's money");
...
assert(persistedGoldTheftSave.log.some((line) => line.includes("倒せば取り返せる")), "gold-theft escape log is missing");
...
document.listeners.keydown({ key: "ArrowRight", preventDefault() {} });
Math.random = trapOriginalRandom;
assert(persistedGoldRecoverySave.adventurer.gold === 100 && persistedGoldRecoverySave.dungeon.enemies[0].stolenGold === 0, "defeating the thief did not recover all stolen gold");
assert(persistedGoldRecoverySave.log.some((line) => line.includes("Gを取り返した")), "stolen-gold recovery log is missing");
